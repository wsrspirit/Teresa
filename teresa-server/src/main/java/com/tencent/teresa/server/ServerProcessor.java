...
public class ServerProcessor implements Processor<IoPacket,IoPacket> {
    private Map<Integer, MethodHanler> methodHanlerMap;
    private static final Logger logger = LoggerFactory.getLogger(ServerProcessor.class);
    private Serializer serializer;

    public ServerProcessor(Map<Integer, MethodHanler> methodHanlerMap,Serializer serializer) {
    ...
    public IoPacket process(IoPacket packet, Channel ioChannel) throws Exception {
        MethodHanler methodHanler = methodHanlerMap.get(packet.getSubcmd());
        //TODO: subcmd not find
        if (methodHanler == null) {
            logger.error("not find methodHandler for subcmd {}",packet.getCmd());
            throw new IllegalArgumentException("not find methodHandler for subcmd:" + packet.getCmd());
        }
        try {
